...
INSERT INTO `accounts` VALUES (1,1,1,'prade516','$2a$10$eqv8.O12TVj8dg9ES3yvyOss/4pMGNYi8CpSHn3OmrDQpAk8henoK',NULL,'1');
...
CREATE TABLE `branchusers` (
  ...
  KEY `idAccount_branchusers` (`idAccount`) /*!80000 INVISIBLE */,
...
INSERT INTO `locations` VALUES (2,1,1,1,1);
...
INSERT INTO `users` VALUES (1,1,2,'32134','Pradel','Eugene','Ovidio Lagos 124','DNI','19032740',1);
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `createBusiness`(
	in businessName varchar(200),in phone varchar(20), in firstName varchar(50), in lastName varchar(50), 
...
begin
    ...
    insert into businesses(businessName, logo,state)
    values(businessName, logo,1);
    SET idBusiness = (SELECT LAST_INSERT_ID());
    
    insert into branches(idBusiness, idLocation, address, state)
    values(idBusiness,idLocation, address, 1);
    SET idBranch = (SELECT LAST_INSERT_ID());
    
    insert into accounts(idRole, idTypeUser, userName, userPass,creationDate, state)
    values(idRole, 1, userName, userPass,getDate(), 1);
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `UpdateUser`(
	in _phone varchar(20), in _firstName varchar(50), in _lastName varchar(50),
	in _address varchar(150), in _typeDocument varchar(20), in _docNumber varchar(20),
    in _idRole int, in _idCountry int, in _idProvince int, 
    in _idcity int, in _idLocation int, in _idUser int)
begin
	update locations set idCountry = _idCountry, idProvince = _idProvince,
					idCity = _idCity where idLocation = _idLocation;
    
    update users set phone = _phone, firstName = _firstName, lastName = _lastName, typeDocument = _typeDocument,
				docNumber = _docNumber, idRole = _idRole where id = _idUser;  
end ;;
DELIMITER ;
/*!50003 SET sql_mode              = @saved_sql_mode */ ;
/*!50003 SET character_set_client  = @saved_cs_client */ ;
...
-- Dump completed on 2021-07-03  0:25:46
