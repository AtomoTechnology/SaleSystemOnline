...
export const GetAll = (req, res) => {
	mysqlconnection.query(
		'SELECT * FROM businesses where state = 1 ORDER BY id DESC',
		(err, rows, fields) => {
			if (!err) {
				res.json(rows);
			} else {
...
export const GetById = (req, res) => {
	const { id } = req.params;
	mysqlconnection.query(
		'SELECT * FROM businesses where state = 1 and id =?',
		[id],
		(err, rows, fields) => {
			if (!err) {
				res.json(rows[0]);
...
export const Put = (req, res) => {
	...
	const { id } = req.params;
	mysqlconnection.query(
		`UPDATE businesses SET businessName = '${businessName}',cuit_cuil = '${cuit_cuil}',logo = '${logo}' WHERE id =${[
			id,
		]}`,
		(err, rows, fields) => {
			if (!err) {
